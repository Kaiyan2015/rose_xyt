...
def checkRefCollection(referenceCollection):
    ...
    if len(namesList) != len(ROSE_utils_cfce.uniquify(namesList)):
...
def main():
    ...
    bamToGFF = pkg_resources.resource_filename(__name__,'ROSE_bamToGFF_cfce.py')
    debug = False
    ...
    #IS there a valid central location?  IF not, use the cwd
    #cwd = os.getcwd()
    #genomeDict = {
    ...
    for bamFile in bamFileList:
        ...
        cmd1 = "python %s -f 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (bamToGFF,nBin,bamFile,stitchedGFFFile,mappedOut1)
        print(cmd1)
        os.system(cmd1)
        ...
        cmd2 = "python %s -f 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (bamToGFF,nBin,bamFile,inputGFFFile,mappedOut2)
        print(cmd2)
        os.system(cmd2)
