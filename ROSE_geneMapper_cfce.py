...
def mapEnhancerToGene(annotFile,enhancerFile,transcribedFile='',uniqueGenes=True,searchWindow =50000,noFormatTable = False):
    ...
    enhancerTable = ROSE_utils_cfce.parseTable(enhancerFile,'\t')
    ...
    if len(transcribedFile) > 0:
        transcribedTable = ROSE_utils_cfce.parseTable(transcribedFile,'\t')
        transcribedGenes = [line[1] for line in transcribedTable]
    else:
        transcribedGenes = startDict.keys()

    print('MAKING TRANSCRIPT COLLECTION')
    transcribedCollection = ROSE_utils_cfce.makeTranscriptCollection(annotFile,0,0,500,transcribedGenes)


    print('MAKING TSS COLLECTION')
    tssLoci = []
    for geneID in transcribedGenes:
        tssLoci.append(ROSE_utils_cfce.makeTSSLocus(geneID,startDict,0,0))
    ...
    for line in enhancerTable:
        ...
        enhancerLocus = ROSE_utils_cfce.Locus(line[1],line[2],line[3],'.',line[0])
        ...
        proximalLoci = tssCollection.getOverlap(ROSE_utils_cfce.makeSearchLocus(enhancerLocus,searchWindow,searchWindow),'both')           
        proximalGenes =[]
        for proxLocus in proximalLoci:
            proximalGenes.append(proxLocus.ID())


        distalLoci = tssCollection.getOverlap(ROSE_utils_cfce.makeSearchLocus(enhancerLocus,1000000,1000000),'both')           
        distalGenes =[]
        ...
        overlappingGenes = ROSE_utils_cfce.uniquify(overlappingGenes)
        proximalGenes = ROSE_utils_cfce.uniquify(proximalGenes)
        distalGenes = ROSE_utils_cfce.uniquify(distalGenes)
        ...
        if noFormatTable:
            ...
            newEnhancerLine.append(join(ROSE_utils_cfce.uniquify([startDict[x]['name'] for x in overlappingGenes]),','))
            newEnhancerLine.append(join(ROSE_utils_cfce.uniquify([startDict[x]['name'] for x in proximalGenes]),','))
    ...
    #Make table by gene
    overallGeneList = ROSE_utils_cfce.uniquify(overallGeneList)  

    #use enhancer rank to order
    rankOrder = ROSE_utils_cfce.order([min(rankDict[x]) for x in overallGeneList])
    ...
    if noFormatTable:
        ...
        enhancerOrder = ROSE_utils_cfce.order([int(line[-2]) for line in enhancerToGeneTable[1:]])
...
def main():
    ...
    if window != 50000:
        ...
        ROSE_utils_cfce.unParseTable(enhancerToGeneTable,out1,'\t')

        #writing the gene table
        out2 = '%s%s_GENE_TO_ENHANCER_%sKB.txt' % (outFolder,enhancerFileName,window/1000)
        ROSE_utils_cfce.unParseTable(geneToEnhancerTable,out2,'\t')
    else:
        #writing the enhancer table
