...
def regionStitching(inputGFF,stitchWindow,tssWindow,annotFile,removeTSS=True):
    ...
    boundCollection = ROSE_utils_cfce.gffToLocusCollection(inputGFF)

    debugOutput = []
    #filter out all bound regions that overlap the TSS of an ACTIVE GENE
    if removeTSS:
        #first make a locus collection of TSS
        startDict = ROSE_utils_cfce.makeStartDict(annotFile)
        ...
        for geneID in startDict.keys():
            tssLoci.append(ROSE_utils_cfce.makeTSSLocus(geneID,startDict,tssWindow,tssWindow))
        ...
        tssCollection = ROSE_utils_cfce.LocusCollection(tssLoci,50)
    ...
    if removeTSS:
        ...
        tssLoci = []
        for geneID in startDict.keys():
            tssLoci.append(ROSE_utils_cfce.makeTSSLocus(geneID,startDict,50,50))
        ...
        for stitchedLocus in stitchedCollection.getLoci():
            ...
            tssNames = ROSE_utils_cfce.uniquify(tssNames)
            if len(tssNames) > 2:
        ...
        fixedCollection = ROSE_utils_cfce.LocusCollection(fixedLoci,50)
...
def mapCollection(stitchedCollection,referenceCollection,bamFileList,mappedFolder,output,refName):
    ...
    lenOrder = ROSE_utils_cfce.order(lociLenList,decreasing=True)
    ticker = 0
    for i in lenOrder:
        ticker+=1
        if ticker%1000 ==0:
    ...
    for bamFile in bamFileList:
        ...
        mappedGFF =ROSE_utils_cfce.parseTable('%s%s_%s_MAPPED.gff' % (mappedFolder,refName,bamFileName),'\t')        
        ...
        for line in mappedGFF[1:]:
            ...
            mappedLoci.append(ROSE_utils_cfce.Locus(chrom,start,end,'.',line[0]))
            try:
        ...
        mappedCollection = ROSE_utils_cfce.LocusCollection(mappedLoci,500)
        locusTable[0].append(bamFileName)

        for i in range(1,len(locusTable)):
            signal=0.0
            line = locusTable[i]
            lineLocus = ROSE_utils_cfce.Locus(line[1],line[2],line[3],'.')
    ...
    ROSE_utils_cfce.unParseTable(locusTable,output,'\t')
...
def main():
    ...
    main run call
    '''
    rscript = pkg_resources.resource_filename(__name__,'ROSE_callSuper.R')
    bamToGFF_turbo = pkg_resources.resource_filename(__name__,'ROSE_bamToGFF_turbo_cfce.py')
    geneMapper = pkg_resources.resource_filename(__name__,'ROSE_geneMapper_cfce.py')
    debug = False
    ...
    outFolder = ROSE_utils_cfce.formatFolder(options.out,True)

    
    #figuring out folder schema
    gffFolder = ROSE_utils_cfce.formatFolder(outFolder+'gff/',True)
    mappedFolder = ROSE_utils_cfce.formatFolder(outFolder+ 'mappedGFF/',True)
    ...
    if options.input.split('.')[-1] == 'bed':
        ...
        ROSE_utils_cfce.bedToGFF(options.input,inputGFFFile)
    ...
    if options.bams:
        ...
        bamFileLIst = ROSE_utils_cfce.uniquify(bamFileList)
    #optional args
    ...
    annotFile = ROSE_utils_cfce.genomeDict[upper(genome)]
    ...
    referenceCollection = ROSE_utils_cfce.gffToLocusCollection(inputGFFFile)
    ...
    stitchedGFF=ROSE_utils_cfce.locusCollectionToGFF(stitchedCollection)
    ...
    if debug:
        ...
        ROSE_utils_cfce.unParseTable(debugOutput,debugOutFile,'\t')

    #WRITE THE GFF TO DISK
    print('WRITING STITCHED GFF TO DISK AS %s' % (stitchedGFFFile))
    ROSE_utils_cfce.unParseTable(stitchedGFF,stitchedGFFFile,'\t')
    ...
    for bamFile in bamFileList:
        ...
        cmd1 = "python %s -e 200 -r -m %s -b %s -i %s -o %s &" % (bamToGFF_turbo,nBin,bamFile,stitchedGFFFile,mappedOut1)
        print(cmd1)
        os.system(cmd1)
        ...
        cmd2 = "python %s 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (bamToGFF_turbo,nBin,bamFile,inputGFFFile,mappedOut2)
        print(cmd2)
        os.system(cmd2)
    ...
    if options.control:
        ...
        cmd = 'R --no-save %s %s %s %s < %s' % (outFolder,outputFile1,inputName,controlName,rscript)
    ...
    cmd = "python %s -g %s -i %s%s" % (geneMapper,genome,outFolder,superTableFile)
    os.system(cmd)    
