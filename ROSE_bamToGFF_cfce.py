...
#def mapBamToGFF(bamFile,gff,sense = 'both',unique = 0,extension = 200,floor = 0,density = False,rpm = False,binSize = 25,clusterGram = None,matrix = None,raw = False,includeJxnReads = False):
    ...
    #USING BAM CLASS
    bam = ROSE_utils_cfce.Bam(bamFile)
    ...
    if ROSE_utils_cfce.checkChrStatus(bamFile) == 1:
      print "has chr"
      hasChrFlag = 1
      #sys.exit();
    else:
    ...
    if type(gff) == str:
        gff = ROSE_utils_cfce.parseTable(gff,'\t')
    ...
    for line in gff:
	  ...
	  line[0] = re.sub(r"chr",r"",line[0])
        gffLocus = ROSE_utils_cfce.Locus(line[0],int(line[3]),int(line[4]),line[6],line[1])
        #print line[0]
        #sys.exit()
        searchLocus = ROSE_utils_cfce.makeSearchLocus(gffLocus,int(extension),int(extension))
        ...
        for locus in reads:
            if locus.sense() == '+' or locus.sense() == '.':
                locus = ROSE_utils_cfce.Locus(locus.chr(),locus.start(),locus.end()+extension,locus.sense(), locus.ID())
            if locus.sense() == '-':
                locus = ROSE_utils_cfce.Locus(locus.chr(),locus.start()-extension,locus.end(),locus.sense(),locus.ID())
        ...
        keys = ROSE_utils_cfce.uniquify(senseHash.keys()+antiHash.keys())
        if floor > 0:
...
def main():
    ...
    if options.input and options.bam:
        ...
        ROSE_utils_cfce.unParseTable(newGFF,output,'\t')
    else:
        parser.print_help()
